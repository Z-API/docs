---
title: Bloqueios e Banimentos (2026)
---
...
## Shadowban (Banimento Temporário do WhatsApp Web)

Durante a utilização da API, pode ocorrer um tipo de restrição conhecido como **shadowban**.

Esse tipo de banimento é diferente do bloqueio tradicional do WhatsApp. Ele é aplicado no contexto do WhatsApp Web, e não representa um banimento definitivo do número.

### Como o shadowban se manifesta

Quando um número está sob shadowban, podem ocorrer os seguintes comportamentos:

- Falha no envio de mensagens (notificado via [webhook de envio](/webhooks/on-message-send))
- Impedimento ao iniciar novas conversas  
- Falha ao enviar mensagens para conversas muito antigas (mesmo que ainda existam)  
- O problema pode ocorrer tanto com contatos salvos quanto não salvos  

### Como identificar o shadowban

É possível identificar esse tipo de restrição através do webhook de [Ao enviar](/webhooks/on-message-send).
...
Ao conectar o número no WhatsApp Web e tentar iniciar uma nova conversa, pode aparecer um aviso informando o tempo restante da restrição, que pode variar de acordo com cada caso.

Não é necessário desconectar a instância, criar uma nova instância ou solicitar desbloqueio ao WhatsApp. Na maioria dos casos, o ideal é apenas aguardar.

#### Exemplo do possível aviso no WhatsApp Web:
<div style={{ display: "flex", justifyContent: "center" }}>
  <img src="/images/shadowban.jpeg" width="500"/>
</div>

### Considerações sobre Shadowban

O shadowban reforça a importância de manter boas práticas de uso, especialmente:

- Evitar envios em massa sem interação  
- Manter comportamento natural do número  
- Estimular respostas dos usuários  

Mesmo sendo temporário, esse tipo de restrição pode impactar operações, principalmente em ambientes com alto volume de envio.
