...
Se você se deparar com qualquer uma dessas telas ao tentar conectar sua instância Z-API, utilize a extensão **Z-API Conector** para o Google Chrome. Por meio dela, é possível autenticar a conexão de forma segura, sem precisar escanear um QR Code.
...
Acesse a Chrome Web Store e instale a extensão **Z-API Conector**:

🔗 https://chromewebstore.google.com/detail/iopmkohngoecmepoheapifkafbeomphn
...
A documentação completa do endpoint:

🔗 https://developer.z-api.io/instance/extension-token

---

## Passo 3 — Inserir o código na extensão

1. Acesse **https://web.whatsapp.com/** no Chrome, estando logado com o número que deseja conectar à instância Z-API
...
Essa nova autenticação foi desenvolvida para se adequar à exigência de autenticação via celular imposta pelo WhatsApp. A extensão Z-API Conector garante que esse processo seja feito de forma segura e integrada à sua instância, sem expor credenciais sensíveis.

Sempre gere um novo token sempre que precisar reconectar a instância, pois cada token possui validade limitada.
